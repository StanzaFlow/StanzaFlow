...
def process_ai_escapes(ir: dict[str, Any], model: str = "gpt-4") -> dict[str, Any]:
    ...
    if escape_blocks:
        ...
        for escape_block in escape_blocks:
            ...
            escape_block["code"] = f"""# AI-generated code (stub)
# Model: {model}
...
def validate_generated_code(code: str, target: str) -> bool:
    ...
    try:
        ...
        raise AIEscapeError(f"Generated code has syntax errors: {e}")
    ...
    class SecurityVisitor(ast.NodeVisitor):
        def visit_Import(self, node):
            ...
            for alias in node.names:
                if alias.name in {'os', 'subprocess', 'sys', 'importlib', '__builtin__', 'builtins'}:
                    dangerous_nodes.append(f"Dangerous import: {alias.name}")
                # Check for aliased dangerous imports
                if alias.asname and alias.name in {'os', 'subprocess', 'sys', 'importlib'}:
                    dangerous_nodes.append(f"Dangerous aliased import: {alias.name} as {alias.asname}")
        
        def visit_ImportFrom(self, node):
            # Check for dangerous from imports
            if node.module in {'os', 'subprocess', 'sys', 'importlib', '__builtin__', 'builtins'}:
        ...
        def visit_Call(self, node):
            ...
            # Check for dangerous method calls
                ...
                if isinstance(node.func.value, ast.Name) and len(node.func.value.id) == 1:
                    if node.func.attr in dangerous_methods:
                        dangerous_nodes.append(f"Suspicious aliased call: {node.func.value.id}.{node.func.attr}")
    ...
    if dangerous_nodes:
        raise AIEscapeError(f"Generated code contains dangerous operations: {', '.join(dangerous_nodes)}")
    ...
    if target == "langgraph":
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom) and node.module and node.module.startswith('langgraph'):
        ...
        if not has_langgraph_imports:
            raise AIEscapeError("Generated LangGraph code must import langgraph modules")
